...
local tempwidget = awful.widget.watch('sensors', 5, function(widget, stdout)
               for line in stdout:gmatch("[^\r\n]+") do
                 if line:match("Tctl") then
                    temps = line:match("+(%d+).*°C") .. "° " -- in Celsius
                    widget:set_markup(markup.font(theme.font, "Tctl: " .. temps))
                   return
                 end
               end
             end)
local tempwidget2 = awful.widget.watch({awful.util.shell, '-c', 'sensors | grep Tctl'}, 30,
function(widget, stdout)
    local temps = ""
    ...
    s.mywibox:setup {
        ...
        { -- Right widgets
           ...
           -- wibox.container.background(wibox.container.margin(wibox.widget { mailicon, mail and mail.widget, layout = wibox.layout.align.horizontal }, 4, 7), "#343434"),
            ...
            arrow("#7E3EBE", "#7E7EFF"),
            wibox.container.background(wibox.container.margin(tempwidget, 4, 4), "#7E7EFF"),
            -- wibox.container.background(wibox.container.margin(wibox.widget { tempicon, tempwidget.widget, layout = wibox.layout.align.horizontal }, 4, 4), "#7E7EFF"),
            --arrow("#7E7EFF", "#7E3EBE"),
            ...
            wibox.container.background(wibox.container.margin(wibox.widget { volume_widget{ widget_type = 'arc', thickness = 4, size = 40}, layout = wibox.layout.align.horizontal }, 2, 3), "#7E3EBE"),
            arrow("#7E3EBE", "#7E7EFF"),
